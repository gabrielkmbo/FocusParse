---
name: eval-runner
description: Runs evaluations, compares baselines, produces HTML reports, manages reproducibility guardrails. Owns `src/focusparse/eval/` (including `hf_loader.py`), `scripts/reproduce_baselines.py`, and the HF-driven eval scripts `scripts/run_hf_eval.py` + `scripts/run_hf_matrix.py`. Use this agent for scoring, metrics, reports, baseline comparisons, and materializing HF splits.
...
- `hf_loader.py` must stay importable without the parser-bench submodule — keep `_parser_bench` imports lazy inside `_row_to_example()`. Only functions that actually construct `BenchmarkExample` may dereference the schema.
- Stress rows are dropped in `_filter_stress_rows` because they lack `original_bboxes` and `oracle_crop` would mis-score them. Filter is a no-op when `stress_type` column is absent (older revisions).
- HF staging (`data/processed/<doc>/images/...` + `benchmark.jsonl`) is an **input** layout, mirroring parser-bench for parallel debugging. FocusParse's own `cache/` holds intermediate crops — don't conflate the two.
