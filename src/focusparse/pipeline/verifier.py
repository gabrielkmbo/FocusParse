...
_SYSTEM_PROMPT = (
    ...
    "any packet summary.\n"
    "- Phase 3e strict-shape rule v2 (2026-05-14 sprint, narrowed): apply "
    "this rule ONLY when ALL of the following are true: (1) `answer_type` "
    "is `exact_match`, (2) the answer is a multi-token phrase of 3+ words "
    "OR contains a comma/semicolon/structural-word like 'page' that "
    "separates fields, AND (3) the document's text in a cited packet "
    "clearly shows that phrase in a SLIGHTLY different shape — extra/missing "
    "space before punctuation (e.g. 'BLE ; Signed' vs 'BLE; Signed'), missing "
    "comma between fields (e.g. 'X.bin 641 kb' vs 'X.bin, 641 kb'), or missing "
    "structural word (e.g. 'Balance Sheets 52' vs 'Balance Sheets, page 52'). "
    "When all three hold, return `next_action=escalate_reasoner` with reason "
    "`'exact_match shape mismatch with cited packet text: <quote>'`. "
    "DO NOT apply this rule when the answer is: a single number ('5', "
    "'6', '70%', '0xFF'), a 1-2 word identifier ('VDD', 'GND', 'Latvia'), "
    "a hex value, a percentage with units, or a measurement value with "
    "units — those should be checked semantically against the cited "
    "packet, not character-for-character. This rule does NOT apply to "
    "numeric / boolean / multiple_choice / unanswerable answer types at "
    "all. When in doubt, prefer `accept` over `escalate_reasoner` — false "
    "rejections on numeric/identifier answers cause the reasoner to "
    "second-guess correct answers into wrong ones.\n"
